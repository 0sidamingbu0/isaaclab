...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformVelocityCommandCfg(
        ...
        ranges=mdp.UniformVelocityCommandCfg.Ranges(
            lin_vel_x=(-2.0, 2.0),  # 【重要可调】前进速度范围(m/s)，(0,1.0)=只前进不后退
...
class RewardsCfg:
    ...
    track_lin_vel_xy_exp = RewTerm(
        func=mdp.track_lin_vel_xy_exp, weight=4.0,  # 权重4.0，越高越重视速度跟踪（主要目标）
    ...
    track_base_height = RewTerm(
        func=mdp.track_base_height_exp, weight=2.0,  # 正权重=2.0，接近目标高度越近奖励越高
    ...
    # 【接触力奖励】- 真正的脚底板水平接触检测
    # 🔧 左脚底板接触奖励 - 结合接触力、脚底角度、法向力方向的综合判断
    feet_contact_forces_LF = RewTerm(
        func=mdp.foot_contact_reward,  # 🔧 使用简化的Z向力接触检测函数
        weight=0.5,  # 适中权重，鼓励正确的接触
        params={
            "sensor_cfg": SceneEntityCfg("contact_forces_LF"),
            "min_force": 2.0,      # 最小有效接触力 2N (过滤轻微碰触)
            "max_force": 200.0,    # 最大合理接触力 200N (过滤撞击)
            "target_force": 50.0,  # 目标接触力 50N (正常支撑力)
            "min_z_ratio": 0.2     # Z向力最小比例 20%，与奖励成正比
        },
    )
    # 🔧 右脚底板接触奖励 - 结合接触力、脚底角度、法向力方向的综合判断
